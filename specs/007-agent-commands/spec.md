...
| Thucydides | TechnicalWriter | Read all (`LIST_*`, `READ_FILE`, `SEARCH_CODE`, `GIT_LOG`, `SHOW_DIFF`); task items + comments; communication (`DM`); memory; evidence (`QUERY_EVIDENCE`, `CHECK_GATES`, `STORE_ARTIFACT`); `SET_PLAN`, `REBASE_TASK`; room navigation (`MOVE_TO_ROOM`, `ROOM_HISTORY`, `ROOM_TOPIC`, `RETURN_TO_MAIN`) | `chat`, `task-state`, `task-write`, `memory`, `code`, `spec-write` (file write scoped to `specs/` and `docs/` only; see "Spec write capability" note below) | `APPROVE_TASK`, `REQUEST_CHANGES`, `RESTART_SERVER` |
