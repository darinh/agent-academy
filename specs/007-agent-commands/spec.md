...
2. **`EnabledTools`** — SDK tool-group gates for non-command capabilities (file write, code execution, task DB mutations): `code` (file read + search), `code-write` (file write), `task-state`, `task-write`, `memory`, `chat`.
...
| Thucydides | TechnicalWriter | Read all (`LIST_*`, `READ_FILE`, `SEARCH_CODE`, `GIT_LOG`, `SHOW_DIFF`); task items + comments; communication (`DM`); memory; evidence (`QUERY_EVIDENCE`, `CHECK_GATES`, `STORE_ARTIFACT`); `SET_PLAN`, `REBASE_TASK`; room navigation (`MOVE_TO_ROOM`, `ROOM_HISTORY`, `ROOM_TOPIC`, `RETURN_TO_MAIN`) | `chat`, `task-state`, `task-write`, `memory`, `code` (spec authorship happens via file edits; see "Spec write capability" note below) | `APPROVE_TASK`, `REQUEST_CHANGES`, `RESTART_SERVER` |

> **Spec write capability**: Thucydides owns the spec (see spec 009 §Spec Ownership) but does not currently hold the `code-write` SDK tool group in `agents.json`. Spec edits route through the human operator or an Engineer acting on Thucydides' proposal. This split keeps spec-file writes under the same review flow as code writes. If Thucydides is granted `code-write` in future, it should be scoped to `specs/` only.
