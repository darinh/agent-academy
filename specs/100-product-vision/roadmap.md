...
- **Refactor candidate: `DiscordNotificationProvider.cs`** (surfaced 2026-04-25 by stabilization gate). 540 lines, 14 fix commits in the last 30 days, **4 lifecycle/concurrency fixes in the last 14 days alone** (#114 connect/dispose hardening, #115 Configure/Connect serialization, #116 outbound-op drain on teardown, plus #112 auth alignment). The connect / configure / dispose state machine is the hot spot. Recommend extracting a dedicated lifecycle/state-machine class so the provider's transport code is decoupled from connection state. **Design doc**: [`discord-lifecycle-refactor-design.md`](./discord-lifecycle-refactor-design.md) (drafted 2026-04-25; APPROVED 2026-04-25 — all §6 design decisions resolved by author). **Why not yet on active roadmap**: not on the critical path to Phase 1 acceptance; current notification path works. Implementation can begin under Anvil Large (3 reviewers, 🔴 risk) at any session — humans may override decisions via PR review on the implementation PR.
