...
| P1.4 | partial | 2026-04-25 | 2026-04-25 | **Blocked-signal subset shipped** (anvil/p1-4-blocked-signal). `SprintEntity.BlockedAt` + `BlockReason` columns; `MarkSprintBlockedAsync`/`UnblockSprintAsync` use atomic `ExecuteUpdateAsync` so concurrent block calls emit at most one `SprintBlocked` event. Sprint stays `Status="Active"` while blocked → workspace-occupancy + idle semantics unchanged. `GetOverdueSprintsAsync` excludes blocked sprints (timeout sweep no longer auto-cancels sprints that are explicitly waiting on a human). `ActivityEventType.SprintBlocked` → `NotificationType.NeedsInput` "Sprint needs attention" (closes the deferred P1.7 bullet). 18 new tests across SprintService + Controller + broadcaster mapping. **Self-evaluation ceremony design drafted** 2026-04-25: [`p1-4-self-evaluation-design.md`](./p1-4-self-evaluation-design.md) — substate-of-Implementation model, `SelfEvaluationReport` artifact with strict per-criterion validation, `RUN_SELF_EVAL` command, verdict path that reuses `MarkSprintBlockedAsync` for the auto-block heuristic. §9 design questions resolved 2026-04-25; ready for implementation per §8 order. **Foundation shipped 2026-04-25 (PR #143)**: `ArtifactType.SelfEvaluationReport` + `SelfEvaluationVerdict`/`SelfEvaluationOverallVerdict`/`SelfEvaluationItem`/`SelfEvaluationReport` records in `AgentAcademy.Shared.Models.Sprints`; 4 additive `SprintEntity` columns (`SelfEvaluationInFlight`/`SelfEvalAttempts`/`LastSelfEvalAt`/`LastSelfEvalVerdict`) + `AddSprintSelfEvaluation` migration; strict static (parse-level) validation in `SprintArtifactService.ValidateArtifactContent` covering required scalars, FixPlan-on-non-PASS, no duplicate TaskIds, OverallVerdict↔rollup consistency, plus a `JsonDocument` pre-pass that closes the missing-enum-field-defaults-to-0 loophole and screens `Items:[null]` with controlled errors (case-insensitive to match the deserializer contract). +32 unit tests; full suite 6403 server + 425 forge. The artifact gate (`RequiredArtifactByStage["Implementation"]`) and the verdict path (RUN_SELF_EVAL command + counters + auto-block heuristic + preamble + API + notifications) are deliberately deferred to the next P1.4 PR — landing the gate before the verdict path would break every in-flight sprint's Implementation→FinalSynthesis advance. |
