...
| P1.4 | partial | 2026-04-25 | 2026-04-25 | **Verdict path shipped** (feat/p1-4-self-eval-verdict, this PR). Behavioral half of the self-evaluation ceremony: `RequiredArtifactByStage["Implementation"] = "SelfEvaluationReport"` + verdict-equality gate (must be `AllPass`) in `SprintStageService.AdvanceStageAsync`; `RUN_SELF_EVAL` command with §4.2 validations + atomic `ExecuteUpdateAsync` flip of `SelfEvaluationInFlight`; single-transaction verdict path in `SprintArtifactService.StoreArtifactAsync` (DB-aware validation: TaskId set ≡ non-cancelled tasks, verbatim `SuccessCriteria` match, `Attempt == prior+1`; counter bump + `LastSelfEvalAt`/`LastSelfEvalVerdict`/`SelfEvaluationInFlight` flip + cap-exceeded auto-block, all under `BeginTransactionAsync` with post-commit broadcasts); append-only artifact storage via filtered partial unique index + covering index migration `20260425150837_SelfEvalArtifactAppendOnly`; `UnblockSprintAsync` resets `SelfEvalAttempts=0`/`SelfEvaluationInFlight=false` on `BlockReason.StartsWith("Self-eval failed")`; `ImplementationSelfEval` preamble selected when `SelfEvaluationInFlight=true` with `{MaxSelfEvalAttempts}` interpolation; new `SelfEvalOptions` (`Orchestrator:SelfEval:MaxSelfEvalAttempts=3`); `ActivityEventType.SelfEvalCompleted` event; `SprintSnapshot` extended with the 4 self-eval fields. +9 new SelfEvalReport tests (verdict-path branches + DB-aware validation + append-only); full server suite 6411 passing. **Foundation shipped 2026-04-25 (PR #143)**: artifact type + records + `SprintEntity` columns + parse-level static validation. **Deferred to PR3**: API endpoints, `SelfEvalCompleted`→Discord notification mapping, and the live acceptance test. **Earlier blocked-signal subset (anvil/p1-4-blocked-signal)**: `SprintEntity.BlockedAt`/`BlockReason` + atomic block/unblock + `SprintBlocked`→`NeedsInput` notification + 18 tests. **Design**: [`p1-4-self-evaluation-design.md`](./p1-4-self-evaluation-design.md) §8 steps 2–7 fully landed in this PR. |
