...
| P1.9 | blocked | 2026-04-26 | — | **Live supervised acceptance run executed against fresh Sprint #3 (cancelled). Verdict: 6 PASS / 2 FAIL / 2 NA — workflow advanced Intake→Planning→Discussion→Validation→Implementation; stalled in Implementation. Two real platform blockers identified, both pre-existing and surfaced by the run, both block Phase 1 closure:** (1) **SDK permission failure** — `unexpected user permission response` on baseline `bash`/`apply_patch` requests in fresh Copilot SDK sessions, reproduced across 2 agents (Hephaestus, Athena) and 2 fresh sessions. The team auto-diagnosed correctly per `AgentPermissionHandler` design and escalated. Root cause likely a kind not in `AlwaysSafeKinds` or `ToolImpliedKinds` — needs SDK-side capture. (2) **Workspace isolation gap** — `write_file` from a breakout-session agent writes to the develop checkout (`/home/darin/projects/agent-academy/`) instead of the assigned task worktree (`.worktrees/task_…/`). Two parallel agents (Hephaestus on backend, Athena on frontend) both wrote to develop simultaneously, producing a cross-task dirty tree (`MM` System.cs, untracked VersionController.cs, modified frontend tsx). The task `BranchName` is set on the Task entity but the server-side cwd / branch-checkout is not switched per agent breakout. Sprint #3 cancelled; dirty develop preserved in stash `anvil-p1-9-cleanup` for inspection. **§10 step results**: 1✓ requirements artifact, 2✓ agent participation, 3✓ sprint plan, 4 NA, 5✓ reached Implementation, 6✗ no self-eval (never reached SelfEval stage), 7✗ no SprintReport, 8✓ room read-only after cancel, 9✓ idle notifications, 10 NA. Acceptance step 6 + 7 fail because of the two blockers above, not because of self-eval/synthesis logic. **Phase 1 is NOT closed until both blockers are fixed and a re-run reaches step 7 PASS.** New roadmap items required: (a) **P1.9-blocker-A: SDK permission diagnostic + fix** — capture the request `Kind` value the SDK emits when this happens (logging in `AgentPermissionHandler.Create` already exists at debug; promote to info during repro and grep `/tmp/aa-server.log` for the unrecognized kind); add it to `ToolImpliedKinds` or `AlwaysSafeKinds`. (b) **P1.9-blocker-B: per-breakout worktree cwd isolation** — the orchestrator must run each breakout's agent execution with cwd set to the assigned worktree (or scope `write_file` against an `allowedRoots` rooted at the task worktree, not `src/`). |
...
- **2026-04-26** (P1.9 blocker B implementation): Implemented per the merged design (PR #160). All three layers fixed: (1) `CodeWriteToolWrapper` + new `CodeReadToolWrapper` accept an optional `scopeRoot` and route writes/reads/commits through it; new shared `ScopeRootValidator` canonicalizes + verifies the scope is an existing git worktree at construction. (2) New `IGitService.CommitStagedInDirAsync` commits in the worktree without `git add -A` (TOCTOU caveat documented inline; not exploitable under the one-agent-per-breakout invariant). (3) All 14 structured command handlers (12 from §2d + `RunFrontendBuildHandler` + `RunTypecheckHandler`, the latter two surfaced by the gpt-5.5 reviewer) honour `context.WorkingDirectory ?? FindProjectRoot()`. Plumbing: `IAgentToolRegistry.GetToolsForAgent` and `CopilotExecutor.CreatePrimedSessionAsync` (and the `_sessionPool.UseAsync` lambda) thread `workspacePath`; `BreakoutCompletionService` threads `worktreePath` into both the reviewer's `RunAgentAsync` (so the reviewer reads the worktree's view) and the fix-round agent's call (closing §4.5). The `code` tool group moved from static to contextual so per-session worktree scope can be captured. Tests: full suite 6561 passing (baseline 6549; +12 new across `WorktreeIsolationTests` — real git worktree end-to-end checks for write/commit/read isolation — plus regression checks on `ReadFileHandler` honoring `WorkingDirectory` and on `AgentToolRegistry` workspacePath plumbing). 3-reviewer adversarial pass (codex + opus + gpt-5.5) converged on `CodeReadToolWrapper` validation gap and the two missed handlers; both addressed before presenting. Awaiting human merge per NO-SELF-MERGE; P1.9 remains `blocked` pending the supervised re-run that reaches §10 step 7 PASS. — agent: anvil (operator: agent-academy)
