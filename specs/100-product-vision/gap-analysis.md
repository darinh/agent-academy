...
- [x] ~~**G5 verification**~~: Closed 2026-04-28 by P2.1 design pass. `SprintSchedulerService.EvaluateSchedulesAsync` (`Services/SprintSchedulerService.cs:74-82`) iterates **every** enabled, due schedule across all workspaces — already cross-workspace. `SprintTimeoutService.CheckOnceAsync` already cross-workspace. `OrchestratorWakeService.WakeWorkspaceRoomsForSprintAsync` resolves the target workspace from `sprint.WorkspacePath`. The actual G5 breakage is **six bugs in shared services that consult `WorkspaceEntity.IsActive`** (the UI's "loaded workspace" hint) when their caller is the per-round dispatch path: planner task-context contamination (`ConversationRoundRunner.cs:187` → `TaskItemService.GetActiveTaskItemsAsync()`); auto-archive predicate eats background-workspace main rooms (`RoomLifecycleService.IsMainCollaborationRoomAsync` returns false for non-active workspace's main rooms); agent SDK `create_task` stamps tasks with loaded workspace (`TaskWriteToolWrapper` → `TaskOrchestrationService.cs:88-104`); `LIST_TASKS` / `list_tasks` filter by loaded workspace (`ListTasksHandler.cs:18-20`, `AgentToolFunctions.cs:117-120` → `TaskQueryService.GetTasksAsync()`); kickoff message naming uses global active workspace; global queue serialisation produces priority inversion. Fixes scoped in [`p2-1-multi-workspace-scheduling-design.md`](./p2-1-multi-workspace-scheduling-design.md). Bugs 1–5 land in the implementation PR; Bug 6 is documented as Phase 3 work. **Gap status**: investigation closed, but G5 itself remains OPEN until the implementation PR lands.
