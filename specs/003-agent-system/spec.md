...
- **Token resolution chain** *(CopilotClientFactory)*: Resolved in priority order via `ResolveToken()`:
...
- ~~**No prompt injection mitigation**~~ — **Mitigated**: `PromptSanitizer` provides defense-in-depth for user-supplied content interpolated into agent prompts via `PromptBuilder`. Three layers: (1) **Boundary markers** — all user-content sections (messages, tasks, DMs, memories, work reports, session summaries) are enclosed in `[UNTRUSTED_CONTENT]`/`[/UNTRUSTED_CONTENT]` delimiters. (2) **Boundary instruction** — a security preamble in every prompt tells the LLM to treat marked content as conversation context, not system-level instructions. (3) **Metadata sanitization** — sender names, room names, and memory keys have newlines/control characters stripped to prevent prompt-structure injection via metadata fields. Marker sequences within content are escaped to prevent marker injection. **Residual gaps**: `ConversationSessionService.GenerateSummaryAsync` builds a summarizer prompt with raw message text that feeds back into later prompts as `sessionSummary` (wrapped at consumption time but not at generation time). Full mitigation would require extending `PromptSanitizer` to the summarizer. Low risk in single-user context.
