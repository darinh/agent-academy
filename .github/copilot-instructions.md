...
4. **Server management**: Always start the server AND client together from the repo root with `npm run dev` (detach=true). This runs `concurrently` over `wrapper.sh --dev` (which invokes `dotnet run --project src/AgentAcademy.Server`) and the Vite client. Restart by killing the wrapper/concurrently process and re-running `npm run dev`.
   ...
   **⚠️ Stale-process trap after `dotnet build` / `dotnet watch` rebuilds.** A long-running `dotnet run` server keeps the *old* compiled assembly in memory even after the on-disk DLL is replaced (you'll see `(deleted)` next to the binary in `/proc/<pid>/exe`). Result: new endpoints landed in HEAD but the running server still 404s them, and `swagger/v1/swagger.json` won't list them. Failure signal: a route from a recent merge returns 404 with auth working everywhere else. Detect with `ls -la /proc/<server-pid>/exe`; if it shows `(deleted)`, the process is stale. Fix: kill the whole tree (concurrently + wrapper.sh + dotnet run + vite) and `npm run dev` from a clean state. **ALWAYS check the swagger doc for newly-merged routes before assuming an endpoint is broken — missing from swagger = stale process, present-but-erroring = real bug.**
